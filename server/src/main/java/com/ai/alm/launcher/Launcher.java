...
public class Launcher {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if ("stop".equals(command)) {
            ...
            out.println("Usage: java -jar <file.jar> [start|stop]\n\t" + "start    Start the server\n\t"
        ...
        try {
            ...
            server.addLifeCycleListener(new LifeCycle.Listener() {
                ...
                public void lifeCycleFailure(LifeCycle lc, Throwable th) {
                    ...
                    err.println("#==> error = " + th.getMessage());
                    th.printStackTrace();
                }

                @Override
                public void lifeCycleStarted(LifeCycle lc) {
                    log.info(String.format("#==> Started jetty @ http://%s:%d/", host, port));
                    out.println(String.format("#==> Started jetty @ http://%s:%d/", host, port));
                }

                @Override
                public void lifeCycleStarting(LifeCycle lc) {
                    log.info(String.format("#==> Starting jetty @ http://%s:%d/ ...", host, port));
                    out.println(String.format("#==> Starting jetty @ http://%s:%d/ ...", host, port));
                }

                @Override
                public void lifeCycleStopped(LifeCycle lc) {
                    log.info(String.format("#==> Stopped jetty @ http://%s:%d/", host, port));
                    out.println(String.format("#==> Stopped jetty @ http://%s:%d/", host, port));
                }

                @Override
                public void lifeCycleStopping(LifeCycle lc) {
                    log.info(String.format("#==> Stopping jetty @ http://%s:%d/ ...", host, port));
                    out.println(String.format("#==> Stopping jetty @ http://%s:%d/ ...", host, port));
                }
    ...
    class DetailedLoggingHandler extends HandlerWrapper {
        ...
        StringBuilder printRequest(long count, WrappedRequest request) throws IOException {
            ...
            b.append('\n');
            // Request line
            prefixId(b, count).append(NOTIFICATION_PREFIX).append("Server in-bound request").append('\n');
            // Method
            ...
            // Entity
            b.append(request.getRequestBody()).append("\n");
            return b;
        }
        ...
        StringBuilder printResponse(long count, WrappedResponse response) throws IOException {
            ...
            b.append('\n');
            // Response line
            prefixId(b, count).append(NOTIFICATION_PREFIX).append("Server out-bound response").append('\n');
            ...
            if (null != content) {
                b.append(response.getContent()).append("\n");
            }
            return b;
        }
